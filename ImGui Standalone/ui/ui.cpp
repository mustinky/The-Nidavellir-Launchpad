...
#include "../utility/font/nidavellir.h"
...
bool UI::CreateDeviceD3D(HWND hWnd)
{
    ...
    sd.BufferDesc.RefreshRate.Numerator = 237;
...
void UI::Render()
{
    ...
    const WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, _T("Nidavellir"), nullptr };
    ::RegisterClassEx(&wc);

    const HWND hwnd = ::CreateWindow(wc.lpszClassName, _T("Nidavellir"), WS_OVERLAPPEDWINDOW, 100, 100, 50, 50, NULL, NULL, wc.hInstance, NULL);
    ...
    io.Fonts->AddFontFromMemoryTTF(nidavellir, 1, 15);
    io.FontDefault = io.Fonts->Fonts[0];

    ImGuiStyle& style = ImGui::GetStyle();

    style.GrabRounding = style.FrameRounding = style.PopupRounding = style.ScrollbarRounding = style.TabRounding = style.ChildRounding = 4.0f;
    ...
    while (!bDone)
    {
        ...
        ImGui::NewFrame();
        {
            build_manager.set_mbl_color();
            noti_manager.update_and_draw();
...
{
    VMProtectBegin("nidavellir::main::handle::program");

    std::thread([]() {
        protect.anti_suspend_check();
        }).detach();
    ;
